...
// Phase 10 (Profile & Business Profile Management): replaces one existing business in place
// (by id) after an edit - unlike addBusiness(), this never appends a new entry, matching
// BusinessService.UpdateBusinessAsync updating the same row rather than creating another one.
export function updateBusinessInState(business) {
...
// Phase 12 (Multiple Business Management): removes one business after a confirmed server-side
// delete. If the deleted business was the active selection, falls back to whatever business is
// now first in the list (or null for a user with none left) - the same "first business is the
// default" convention setBusinesses() already uses at boot.
export function removeBusinessFromState(businessId) {
...
// Bulk-hydrates applications from the real backend (js/api.js's bootstrapSession()) - used only
// at boot, to replace SAMPLE_APPS with the logged-in user's actual applications (an empty
// array for a user who hasn't submitted any yet - dashboard.js/myApplications.js already
...
// Phase 7: `app` is the real ApplicationResponseViewModel returned by POST
// /api/applications/save (Controllers/ApplicationController.cs) - id/caseId/status/stage are
// all server-assigned now (the CaseId is derived from the database identity column, so it's
// guaranteed unique - no more client-side random-number generation). `documents` stays a
// separate, purely client-side param since there's still no document-upload endpoint.
export function addApplication(app, documents) {
  const shortDate = new Date(app.submittedDate).toLocaleDateString("en-US", { month: "short", day: "numeric" });

  state.applications = [app, ...state.applications];
  state.bankApplications = [
    { id: app.id, caseId: app.caseId, business: app.businessName, scheme: app.scheme, amount: app.amount, status: "pending", submitted: shortDate, risk: "Low" },
    ...state.bankApplications,
  ];
  if (documents && documents.length) {
    state.applicationDocuments = { ...state.applicationDocuments, [app.id]: documents };
  }
...
// Phase 11 (Notifications & Communication Module): bulk-hydrates real notifications from the
// backend (js/api.js's listMyNotifications()) - used at boot, replacing SAMPLE_NOTIFICATIONS
// with the logged-in user's actual rows (newest first, as returned by the server).
export function setNotifications(list) {
  state.notifications = list;
  notify();
}

// Marks a single notification read locally after the server confirms the write
// (POST /api/notifications/{id}/read) - unlike markNotificationsRead() above, this never
// touches the others, matching NotificationService.MarkAsReadAsync's single-row update.
export function markOneNotificationRead(id) {
